...
learningrate = 2e-4
history_max_len = 2600  # 模型记忆的最大token长度
top_p = 0.9
temperature = 0.35
...
target_modules = find_all_linear_names(model,'lora')
# 初始化lora配置
config = LoraConfig(
    r=64,
    lora_alpha=16,
...
stopping_criteria = StoppingCriteriaList([KeywordsStoppingCriteria(["</s>"])])
...
def 保存适配器(path):
    model.eval()
    adapter_state_dict = model.base_model.model.adapter_model.state_dict()
    torch.save(adapter_state_dict, path+"/adapters.bin")
    print('模型保存成功')
函子={"更改学习率":更改学习率,"保存模型":保存模型,"保存适配器":保存适配器,"问答":qa,"训练":训练,"江松训练":江松训练,"连续对话":连续对话,"清空对话":清空对话}#接受一个变量的函数字典
函丑={}#接受两个变量的函数字典，依此类推
函寅={}
函卯={}
函辰={}
